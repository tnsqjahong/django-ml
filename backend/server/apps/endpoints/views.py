...
from server.wsgi import registry, sdRegistry
...
from apps.endpoints.models import SDAlgorithm
from apps.endpoints.serializers import SDAlgorithmSerializer

from apps.endpoints.models import SDAlgorithmStatus
from apps.endpoints.serializers import SDAlgorithmStatusSerializer

from apps.endpoints.models import SDRequest
from apps.endpoints.serializers import SDRequestSerializer
...
class PredictView(views.APIView):
    def post(self, request, endpoint_name, format=None):
        ...
        print(registry, registry.endpoints)
...
class ABTestViewSet(
    mixins.RetrieveModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet, mixins.CreateModelMixin, mixins.UpdateModelMixin
):
...
class StopABTestView(views.APIView):
    def get(self, request, ab_test_id, format=None):
        try:
            ...
            print(all_responses_2, correct_responses_1, accuracy_2)
...
class ImageCreateView(views.APIView):
    def post(self, request, endpoint_name, format=None):
        algorithm_status = self.request.query_params.get("status", "production")
        algorithm_version = self.request.query_params.get("version")
        # algs = SDAlgorithm.objects.filter(status__status = algorithm_status, status__active=True)
        # if algorithm_version is not None:
        #     algs = algs.filter(version = algorithm_version)
        
        # if len(algs) == 0:
        #     return Response(
        #         {"status": "Error", "message": "SD algorithm is not available"},
        #         status=status.HTTP_400_BAD_REQUEST,
        #     )
        # alg_index = 0
        
        # algorithm_object = sdRegistry.endpoints[algs[alg_index].id]
        algorithm_object = SDImageCreator()
        image = algorithm_object.create_image(request.data)
        ...
        # image["request_id"] = sd_request.id
